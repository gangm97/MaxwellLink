...
def calc_transverse_components_3d(size = (20, 20, 20), dx = 1.0, sigma = 1.0, mu12 = 0.10, local_size=100, component="z"):
    """
    ...
    """
    filename = f"_transverse_polarization_{component}_{size}_{dx}_{sigma}_{mu12}_{local_size}.npz"
    ...
    try:
        ...
        Py_t = data["Py_t"]
        Pz_t = data["Pz_t"]
        print("### Loaded transverse polarization from local disk.")
    ...
    Pz = 0.0 * Z
    if component == "z":
        Pz = (mu12 / (2.0 * np.pi)**(1.5) / sigma0**5) * Z**2 * np.exp(-R2 / 2.0 /sigma0**2)
    elif component == "x":
        Px = (mu12 / (2.0 * np.pi)**(1.5) / sigma0**5) * X**2 * np.exp(-R2 / 2.0 /sigma0**2)
    elif component == "y":
        Py = (mu12 / (2.0 * np.pi)**(1.5) / sigma0**5) * Y**2 * np.exp(-R2 / 2.0 /sigma0**2)

    Pz0 = (mu12 / (2.0 * np.pi)**(1.5) / sigma0**5) * Z**2 * np.exp(-R2 / 2.0 /sigma0**2)
    ...
    FPx_t = (1.0 * RF - Kx * Kx / (K2 / RF)) * FPx + (0.0 * RF - Kx * Ky / (K2 / RF)) * FPy + (0.0 * RF - Kx * Kz / (K2 / RF)) * FPz
    FPy_t = (0.0 * RF - Ky * Kx / (K2 / RF)) * FPx + (1.0 * RF - Ky * Ky / (K2 / RF)) * FPy + (0.0 * RF - Ky * Kz / (K2 / RF)) * FPz
    FPz_t = (0.0 * RF - Kz * Kx / (K2 / RF)) * FPx + (0.0 * RF - Kz * Ky / (K2 / RF)) * FPy + (1.0 * RF - Kz * Kz / (K2 / RF)) * FPz
    ...
    start_idx_z, end_idx_z = int((local_size - size[2])/2/dx) , int((local_size + size[2])/2/dx)
    print(start_idx_z, end_idx_z)
    ...
    # save them to local disk to avoid recalculation
    np.savez(filename,
             Pz=Pz,
             Px_t=Px_t,
             Py_t=Py_t,
