...
def get_dipole(filename):
    ...
    t, mux, muy, muz = data[nst:nend:nskip,0], data[nst:nend:nskip,2], data[nst:nend:nskip,3], data[nst:nend:nskip,4]
    mux -= mux[0]
    muy -= muy[0]
    muz -= muz[0]
    t_fs = t * au2fs
    ...
    sp_tot = -freq * (np.imag(spz) + np.imag(spy) + np.imag(spx)) # dipole velocity form
...
def plot_dynamics():
    # get real-time data
    path_outcav = "./"
    t_outcav, mu_outcav, freq_outcav, sp_outcav = get_dipole(filename=path_outcav + "rt_tddft_energy_dipoles.txt")

    fig, axes = clp.initialize(1, 2, width=8, height=4, return_fig_args=True,
        fontsize=12, LaTeX=False, labelthem=True, labelthemPosition=[0.1, 0.963], labelsize=13)

    labels = ["RT-TDDFT", "LR-TDDFT"]
    colors = [clp.navy_blue, "k--"]

    x1s, y1s = [t_outcav], [mu_outcav]
    clp.plotone(x1s, y1s, axes[0], colors=colors, labels=labels, lw=0.5,
             ylabel=r"$\mu_z^{\rm e}(t)$ [a.u.]", showlegend=False, yscientific=True)

    x2s, y2s = [freq_outcav, freq_lr], [sp_outcav / np.max(sp_outcav), e_sp_lr / np.max(e_sp_lr)]
    clp.plotone(x2s, y2s, axes[1], colors=colors, labels=labels, lw=1,
